...
#include "alg_classes/bhv_basic_pass.h"
#include "alg_classes/bhv_basic_shoot.h"
...
Bhv_BasicOffensiveKick::execute( PlayerAgent * agent )
{
    ...
    // 0: shoot , 1: pass , 2: driblle , 3: clearball
    std::vector<int> order = { 2, 0 , 1 , 3 }; 
    
    for(int i = 0 ; i < 4 ; i++)
    {
        if(order[i] == 0)
        {
            return BHV_Basic_shoot().execute(agent);
        }
        if(order[i] == 1)
        {
            return BHV_Basic_pass().execute(agent);
        }
        if(order[i] == 2)
        {
            return dribble(agent);
        }
        if(order[i] == 3)
        {
            return clearball(agent);
        }
    }
